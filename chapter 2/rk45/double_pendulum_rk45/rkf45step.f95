...
subroutine rkf45step(t,y,h)  ! 4-th order Runge-Kutta step
    ...
    contains
        ...
        subroutine deriv(t,h,y,k)   ! derivative
	        ...
	        use setup, only : dp, n_eq, g, length, mass
	        implicit none
	        real(dp), intent(in) :: t, h
	        real(dp), dimension(n_eq), intent(in) :: y
	        real(dp), dimension(n_eq) :: f, k

	        f(1) = y(2)/(mass*length**2)
	        f(2) = -mass*g*length*sin(y(1))	        
	        !f(3) = -mass*g*length*sin(y(1))*.01!multiplying by .01 just moves the scale
	        !f(4) = -mass*g*length*cos(y(1))*.01
