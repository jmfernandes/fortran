...
program pendulum
	...
	real(dp) :: y(n_eq), t, dt, tmax, lambda
...
! 	y(4) = 2._dp				! initial momentum 2
	...
	open(unit = 3, file='pendulum1_point.data', action = 'write', status = 'replace')
	open(unit = 4, file='pendulum2_point.data', action = 'write', status = 'replace')
	open(unit = 5, file='pendulum1_angle.data', action = 'write', status = 'replace')
	open(unit = 6, file='pendulum2_angle.data', action = 'write', status = 'replace')


	do while( t <= tmax )
		
		write(3, fmt = '(2f20.10)') y(1), y(2)
		write(4, fmt = '(2f20.10)') y(3), y(4)
		write(5, fmt = '(2f20.10)') t, y(1)
		write(6, fmt = '(2f20.10)') t, y(3)
		call rk4step(t,y,dt)
